...
// Type System Implementation
	...
	ss.initClass = function ss$initClass(ctor, members, baseType, interfaces) {
		ctor.__class = true;
	...
	ss.initGenericClass = function ss$initGenericClass(ctor, typeArgumentCount) {
		ctor.__class = true;
		ctor.__typeArgumentCount = typeArgumentCount;
		ctor.__isGenericTypeDefinition = true;
	};

	ss.initInterface = function ss$initInterface(ctor, members, baseInterfaces) {
	...
	ss.initGenericInterface = function ss$initGenericClass(ctor, typeArgumentCount) {
		ctor.__interface = true;
		;
		ctor.__typeArgumentCount = typeArgumentCount;
		ctor.__isGenericTypeDefinition = true;
	...
	ss.initEnum = function ss$initEnum(ctor, members) {
		ss.shallowCopy(members, ctor.prototype);
	...
	ss.getAttributes = function ss$getAttributes(type, attrType, inherit) {
		...
		if (inherit) {
			...
			if (b)
				result = ss.getAttributes(b, attrType, true).filter(function(a) {
					var t = ss.getInstanceType(a);
					return !t.__metadata || !t.__metadata.attrNoInherit;
				});
		}
...
// Stopwatch
	...
	if (typeof(window) !== 'undefined' && window.performance && window.performance.now) {
		...
		ss_Stopwatch.isHighResolution = true;
		ss_Stopwatch.getTimestamp = function() {
...
// CultureInfo
	...
	ss.initClass(ss_CultureInfo, {
		getFormat: function CultureInfo$getFormat(type) {
			switch (type) {
			case ss_NumberFormatInfo:
				return this.numberFormat;
			case ss_DateTimeFormatInfo:
...
// IDictionary
	...
	ss.initInterface(ss_IDictionary, { get_item: null, set_item: null, get_keys: null, get_values: null, containsKey: null, add: null, remove: null, tryGetValue: null }, [ss_IEnumerable]);
...
// MutableDateTime
	...
	ss.JsDate = ss_JsDate;
	ss.initClass(ss_JsDate, {}, Object, [ss_IEquatable, ss_IComparable]);
